function [ T_next,x ] = insta_ray(fig,cfl,T1,T0,Tinf,L,h,Eps,n1,n2,n3)
%INSTA_RAY Summary of this function goes here
...
var=1;
n=-1;
Sig=5.68e-8; %constante de stephan

while var>1e-5
        n=n+1;
        T_next(1)=T0;
        for i=2:99
            T_next(i)=T(i)+cfl*(T(i-1)+T(i+1)-2*T(i))-((4*h*dt)/(rho*cp*e))*(T(i)-Tinf)-((4*Sig*Eps*dt)/(e*rho*cp))*(T(i)^4-Tinf^4);
        end
        T_next(100)=T1;
